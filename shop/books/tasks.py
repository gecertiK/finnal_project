    ...
    while True:
        ...
        try:
            for elements in soup['results']:
                if not Book.objects.filter(store_book_id=elements['id']).exists():
...
def update_count_books():
    ...
    while True:
        ...
        try:
            for elements in soup['results']:
                if not Book.objects.filter(store_book_id=elements['id'], count=elements['count_value']):
                    Book.objects.filter(store_book_id=elements['id']).update(count=elements['count_value'])
        except TypeError:
            return print('TypeError')
        try:
            page = soup['next'].split('/')[-1]
