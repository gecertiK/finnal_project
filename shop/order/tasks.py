...
@shared_task
def order_to_store(customer_email, transaction_id):
    print(f'Sending order with transaction_id:{transaction_id} to store')
    ...
    try:
        requests.post(
            'http://store:8000/api/order_item/',
            headers={'Authorization': f'Token {token_value}'},
    ...
    for item in address:
        ...
        try:
            requests.post(
                'http://store:8000/api/shipping_address/',
                headers={'Authorization': f'Token {token_value}'},
